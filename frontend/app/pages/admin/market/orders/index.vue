...
<template>
  <div>
    ...
    <p class="mt-1.5 text-sm text-muted-foreground">Oversight only. You cannot change order status here.</p>
    <DiyMarketTabs
      :items="[
    ...
    <div v-else class="space-y-3">
      <NuxtLink v-for="order in orders" :key="order.id" :to="`/admin/market/orders/${order.id}`">
