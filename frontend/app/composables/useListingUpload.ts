...
export function useListingUpload() {
  ...
  async function uploadPhotos(listingId: string, files: File[]): Promise<ListingPhoto[]> {
    if (!user.value) {
    ...
    for (const [i, file] of files.entries()) {
      ...
      const path = `${user.value.id}/${listingId}/${crypto.randomUUID()}.${ext}`
  ...
  async function getSignedUrls(photos: ListingPhoto[]): Promise<ListingPhoto[]> {
    if (photos.length === 0) {
      return []
    }
